...
<div class="content-wrapper">
    ...
    <section class="content" id="calendarDiv">
      ...
      <a class="btn btn-dark" href="{{ Route('calendar',session('business_name')) }}">{{ __('keywords.location_wise') }}</a>
      <div id='calendar'></div>
    </section>
</div>
@stop
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      resourceAreaHeaderContent: "{{ __('treatment.users') }}",
      ...
      resources: [
        @foreach($users as $user)
            @foreach($businesses as $business)
                @php
                    $userCheck = App\Models\User::where('is_active',1)->where('role','!=','Customer')->where('email',$user->email)->where('business_id',$business->id)->first();
                @endphp
                @if($userCheck != NULL)
                  { id: '{{ $business->id.$user->email }}', title: '{!! ucwords($business->brand_name) ?: ucwords($business->business_name) !!} ({{ $business->business_name.'.'.config('app.domain')  }})', location: '{!! ucwords($user->name) !!} ( {{ $user->email }})'},
                @endif
            @endforeach
        @endforeach
      ],
      ...
      events: [
        @foreach($users as $user)
            @foreach($businesses as $business)
                @php
                    $user_id = App\Models\User::where('is_active',1)->where('role','!=','Customer')->where('email',$user->email)->where('business_id',$business->id)->first();
                @endphp
                @if($user_id != NULL)
                    @foreach($business->Dates->where('is_active',1)->where('user_id',$user_id->id) as $date)
                        @php
                        ...
                        { id: '{{ $date->id }}', resourceId: '{{ $date->business_id.$user_id->email }}', start: '{{ \Carbon\Carbon::parse($date->date)->format("Y-m-d") }}T{{ $newstarttime}}:00', end: '{{ \Carbon\Carbon::parse($date->date)->format("Y-m-d") }}T{{ $newendtime }}:00', title: '{{ $date->from.' - '.$date->till }}', description: '{{ __("treatment.lunch") }}: {{ ($date->treatmentSlotLunchTime ? $date->treatmentSlotLunchTime->time : "N/A") }} \n {{ __("treatment.treatments") }}: {{ $treatments }}'},
                    @endforeach
                @endif
            @endforeach
        @endforeach
