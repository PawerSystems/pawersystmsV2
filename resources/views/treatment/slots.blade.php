...
/* .Absence{ background:#ff0707 !important; }
.LateCancellation{ background:#d16417 !important; }
.PaymentDone{ background:#ffee07 !important; }
.PaymentPending{ background:green !important; }

.AbsenceDot{ color:#ff0707 !important; }
.LateCancellationDot{ color:#d16417 !important; }
.PaymentDoneDot{ color:#ffee07 !important; }
.PaymentPendingDot{ color:green !important; } */
</style>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{ __('treatment.treatment_dates') }}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">{{ __('keywords.home') }}</a></li>
              <li class="breadcrumb-item active">{{ __('treatment.treatment_dates') }}</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content treatment">
      <div class="container-fluid">
        <div class="row">
          <x-bookingtable dimention='>=' therapist="{{ $id }}" />
        </div>
      </div>  
    </section>    
  </div> 
      ...
      success: function (data) {
        ...
        if(data['status'] == 'success'){
          ...
          jQuery(tr).find('td').each(function(){
              ...
              if(jQuery(this).hasClass('status')){
                jQuery(this).removeClass('bg-success').addClass('bg-dark');
                jQuery(this).html('{{ __("treatment.waiting_list") }}');
              }
              ...
              if(jQuery(this).hasClass('bookDel')){
                jQuery(this).html('<button type="submit" class="btn btn-danger" data-time="'+rtime+'" data-id="'+data['slot_id']+'" onclick="waitingSlotDelete(this)">{{ __('treatment.delete_booking') }}</button>');
              }
          });
          
          //-------- Make waitinglist tr ------
          var html = getWaitingTrHtml(rtime,dateId,data['treatments'],data['departments']);
      ...
      success: function (data) {
        ...
        if(data['status'] == 'success'){
          ...
          if({{ $mobilePay }} && data['price'] > 0)
            mobilePayBtn = '<a href="javascript:;" data-slot-id="'+data['slot_id']+'" class="btn btn-sm btn-default btn-block mt-1" onclick="sendMobilePayReq(this)"><img src="/images/mobilepay.svg" alt="MobilePay" width="100"></a>';
...
function waitingSlotDelete(obj){
    ...
    $.ajax({
      type: 'POST',
      url: '/DeleteBookWaitingTimeSlot',
      data: {"_token":token,"id":slotId},
      dataType: 'json',
      success: function (data) {
        if(data['status'] == 'success'){

          //------- Delete waiting list option ------
          jQuery(tr).remove();
  ...
  $.ajax({
      ...
      success: function (data) {
        if(data['status'] == 'success'){
          ...
          var department = getDepatments(data['departments']);
          ...
          jQuery(tr).find('td').each(function(){
              if(jQuery(this).hasClass('status')){
                ...
                jQuery(this).find('span').text('{{ __("treatment.available") }}');
          ...
          //------- Delete waiting list option ------
          jQuery('#form0000'+data['date_id']).closest('tr').remove();
...
function getTrHtml(time,dateID,treatments,departments){
  ...
  var department = getDepatments(departments);
...
function getWaitingTrHtml(time,dateID,treatments,departments){
  ...
  var tr = '<tr>';
  tr += '<td class="bg-secondary text-center status"><form id="form'+ntime+dateID+'"><input type="hidden" name="date_id" value="'+dateID+'" /><input type="hidden" name="data_time" value="'+time+'" /></form><span> {{ __('treatment.waiting_list') }}</span></td><td class="text-center time">'+time+'</td><td class="name"><input type="text" class="form-control" name="name" placeholder="{{ __('keywords.name') }}" data-time="'+time+'" date-id="'+dateID+'" onkeyup="suggestUser(this)" autocomplete="off" /><ul class="list-group p-a" id="name'+ntime+dateID+'"></ul></td><td class="email"><input type="email" class="form-control" name="email" placeholder="{{ __('keywords.email') }}" data-time="'+time+'" date-id="'+dateID+'" onkeyup="suggestUser(this)" autocomplete="off" /></td><td class="number"><input type="text" class="form-control" name="number" placeholder="{{ __('keywords.number') }}" data-time="'+time+'" date-id="'+dateID+'" onkeyup="suggestUser(this)" autocomplete="off" /></td><td class="text-center clipCard"></td><td class="text-center cutBack"></td><td class="text-center cutCard"></td><td class="department"><select name="department" class="form-control select2 " data-time="'+time+'" >'+departments+'</select></td><td class="treatment"><select name="treatment" class="form-control select2 " data-time="'+time+'" >'+options+'</select></td><td class="text-center bookTime"></td><td class="comment"><input type="text" class="form-control" name="comment" placeholder="{{ __('treatment.comment') }}" data-time="'+time+'" /></td><td class="text-center bookDel"><button type="submit" class="btn btn-info" data-time="'+time+'" date-id="'+dateID+'" onclick="bookWaitingSlot(this)">{{ __('treatment.book') }}</button></td><td class="text-center pauseUnpause">'+closeTime+'</td>';
  tr += '</tr>';
...
function getDepatments($departments){
    var options;
...
<script>
    ...
    $.ajax({
      ...
      success: function(response){
          ...
          if(response['status'] == 'success'){
            ...
            //-------- If clips has book then don't allow to change treatment -----//
            var txt = jQuery(obj).closest('tr').find('.treatment').text();
            jQuery(obj).closest('tr').find('.treatment').html(txt);
    ...
    $.ajax({
      ...
      success: function(response){
          ...
          if(response['status'] == 'success'){
            ...
            //-------- If clips has been cut back then allow to change treatment -----//
            var txt = jQuery(obj).closest('tr').find('.treatment').text();
            var bookingId = jQuery(obj).closest('tr').find('.bookDel button').attr('data-id');
            txt = '<a href="javascript:;" title="{{ __('keywords.click_to_change') }}" data-id="'+bookingId+'" onclick="getAvailableTreatments(this)">'+txt+'</a>';
            jQuery(obj).closest('tr').find('.treatment').html(txt);
  ...
  jQuery('.show_all_free').on('click',function(){
    jQuery('td.bg-warning, td.bg-info').parent('tr').hide();
  }); 
 
  jQuery('.show_all').on('click',function(){
    jQuery('td.bg-warning, td.bg-info ').parent('tr').show();
    jQuery('.ther-*').show();
  }); 
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if(response == 1){
          ...
          jQuery(obj).html('<i class="fa fa-check magin-color-icon"></i><img src="/images/mobilepay.svg" alt="MobilePay" width="100">');
        }
        else{
          Toast.fire({
              icon: 'success',
