...
<svg id="svg" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 590 484.7">
  ...
  <g id="envelope">
	...
	<div class="bottom">
		<h2 class="title">{{ __('customer.do_you_want_to_unsub') }}</h2>
		<p class="subtitle">{{ __('customer.iyuywsron') }}</p>
			<div class="buttons">
				<a id="unsubscribe" onclick="unsubUser({{$user->id}})" >{{ __('customer.unsubscribe') }}</a>
                
				<a href="//{{ $business->business_name }}.{{ \Config::get('app.domain') }}" id="cancel">{{ __('customer.cancel') }}</a>
			</div>
	</div>
</div>
    ...
    $.ajax({
      ...
      success: function (data) {
        if(data['status'] == 'success'){
            jQuery('.bottom .title').text("{{ __('customer.we_are_sad_to_see_you_go') }}");
            jQuery('.bottom .subtitle').text("{{ __('customer.yhbuawnlhfu') }}");
            jQuery('.bottom .buttons').remove();
        }
      },
      error: function (data) {
