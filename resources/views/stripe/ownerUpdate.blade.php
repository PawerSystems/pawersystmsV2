...
<section class="content-wrapper">
    <div class="container">
        ...
        <form action="{{ route('subscription.owner_update',session('business_name')) }}" method="POST" id="subscription-form">
            @csrf
            ...
            <div class="form-group">
                <label for="owner">{{ __('subscription.select_owner') }}</label>
                <select name="owner" id="owner" class="form-control">
                    @foreach ($admins as $admin)
                        <option value="{{ $admin->id }}" {{ $admin->id == auth()->user()->id ? 'selected' : '' }}>{{ $admin->name }}</option>
                    @endforeach
                </select>
                @if ($errors->has('owner'))
